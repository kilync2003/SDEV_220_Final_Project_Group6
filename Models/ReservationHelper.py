...
def make_reservation():
    ...
    c.execute("SELECT * FROM reservations WHERE reservation_time = ?",
    ...
    if existing_reservation:
        ...
        c.execute("INSERT INTO reservations VALUES (?, ?, ?)",
                  (reservation_time_str, name, num_guests))
        conn.commit()
        print("Reservation confirmed for", name, "on", date_str,
              "at", time_str, "for", num_guests, "guests.")
...
def modify_reservation():
    ...
    if existing_reservation:
        ...
        new_num_guests = int(
            input("Enter the new number of guests (or 0 to cancel): "))
        if new_num_guests == 0:
            c.execute(
                "DELETE FROM reservations WHERE reservation_time = ?", (reservation_time_str,))
            conn.commit()
            print("Reservation canceled.")
        else:
            c.execute("UPDATE reservations SET num_guests = ? WHERE reservation_time = ?",
                      (new_num_guests, reservation_time_str))
            conn.commit()
            print("Reservation updated for", name,
                  "with", new_num_guests, "guests.")
    else:
...
while True:
    choice = input(
        "Enter 1 to make a reservation, 2 to modify a reservation, or 0 to exit: ")
